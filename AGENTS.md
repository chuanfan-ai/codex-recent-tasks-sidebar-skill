...
WorkBuddy 与 TRAE Work 统一通过 `AgentAdapter.swift` 管理，并在界面中作为与 Codex、QwenWorkCN、Kimi 平级的独立产品栏展示；产品名称使用相同字号和字重，不增加分组标题、模式说明或可见的“待适配”徽标。WorkBuddy 仅在已验证的 5.3.5 版本上，通过当前用户私有 sidecar 提供的公开 loopback REST 契约读取有界会话摘要；自动化验收不得回传真实名称或 ID。TRAE Work 在没有稳定外部只读契约时只检查应用路径、Bundle ID、版本和运行状态。不得读取两款产品的用户目录、日志、数据库、DOM、调试端口、账号、凭证或会话正文。外部灰测任务还需读取 `docs/FIRST_EXTERNAL_GRAY_PLAN.zh-CN.md`。
