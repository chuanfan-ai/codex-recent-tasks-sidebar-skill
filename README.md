...
- 从 `~/.codex/.codex-global-state.json` 只读匹配 Codex 为当前顶层任务记录的未读更新；显示绿色“待查看”，表示有新回复尚未查看，不代表任务结束。内部子线程残留不会被误算到顶层任务。
...
- 绿色“待查看”准确含义是“当前顶层任务有新回复尚未查看”；它会随 Codex 的已读状态在刷新后消失，不表示对话完成或归档，也不会受内部子线程残留未读影响。
