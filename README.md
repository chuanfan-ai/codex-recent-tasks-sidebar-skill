...
正式构建会先把源码复制到 `/private/tmp` 下的独立临时目录再编译，结束后自动清理，避免把打包机 Home 绝对路径写入 App 二进制。`qa.sh` 会把该路径脱敏检查作为阻断项。
...
验收包含原生构建、Swift 严格并发检查、应用名称与版本检查、临时签名、架构检查、打包机 Home 路径阻断、三类 Agent 的固定合成测试库、自检、Kimi Work 三态与未读解析、Work/CLI 独立回退、额度解析与故障恢复、Kimi 总量合并与“总量 / Code 5h / Code 7天”窄栏展示、QwenWorkCN 非消费式待查看读取、两款产品的官方图标、WorkBuddy 运行状态优先发布与启停事件即时刷新、公开会话解析与版本闸门、TRAE IDE 排除、平级展示契约、故障隔离、输入文件只读哈希比对和脱敏扫描。
