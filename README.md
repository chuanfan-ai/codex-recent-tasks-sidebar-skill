...
- 单独置顶模式保持全局前置并可自由拖动。
- 默认显示 Codex 剩余用量百分比和每周额度的本机时区重置时间；官方服务单次失败时保留上次成功数据并自动重试，连续失败才显示暂时不可用，且始终不影响任务列表。
...
- 剩余用量通过 ChatGPT / Codex 自带的官方 `app-server` 和当前登录状态联网读取；App 只在内存中保留用量百分比和重置时间，不读取、输出或保存 Token、原始账号响应或其他账号信息。
