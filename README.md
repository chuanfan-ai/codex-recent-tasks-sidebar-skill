...
一个常驻桌面的原生 macOS 小工具，用 240 像素窄栏查看本机 AI Agent 状态。Codex、QwenWorkCN 和 Kimi 已接入活动线程与额度；首批 Cowork 产品 WorkBuddy、TRAE Work 已接入隐私安全的安装与运行状态发现。
...
- “首批 Cowork”分区显示 WorkBuddy 与 TRAE Work 是否安装、是否运行和支持等级；两款产品均为“已发现 · 待适配”，不显示也不推测任务或额度。
