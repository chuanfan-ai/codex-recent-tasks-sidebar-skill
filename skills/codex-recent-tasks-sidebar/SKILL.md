...
- Interpret blue “运行中” from the latest unmatched `task_started`, orange “待操作” from an unfinished explicit user-input or approval request, and green “待查看” only when the task is no longer active and its own thread ID has an unread update. Read `electron-thread-read-state-v1.unreadByIdentity` from the top level of `.codex-global-state.json`, falling back to `electron-persisted-atom-state.unread-thread-ids-by-host-v1` only when the new field is absent. Match both formats directly and never promote a child thread's unread state to its parent. These labels never mean the conversation is permanently completed or archived.
