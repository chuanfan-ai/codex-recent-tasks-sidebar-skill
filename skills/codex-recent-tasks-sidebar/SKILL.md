...
description: Build, customize, validate, or repair a native macOS Codex recent-tasks companion window. Use when a user wants a Dock-visible SwiftUI utility that reads recent Codex tasks, unread update state, and remaining usage, groups tasks by working folder, shows activity times, opens exact tasks, docks on either side of Codex, follows Codex foreground state, or stays independently pinned.
---
...
5. Launch the app and verify the real UI:
   ...
   - a task with an unread update shows a green “待查看” label beside its activity time; opening it clears the label after refresh and does not imply the conversation is finished;
...
- Treat the Codex SQLite database, `session_index.jsonl`, and `.codex-global-state.json` as read-only. Never migrate, vacuum, replace, upload, or write to them.
