...
[[ "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleDisplayName' "$APP_DIR/Contents/Info.plist")" == "本机AI状态栏" \
   ...
   && "$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "$APP_DIR/Contents/Info.plist")" == "14" ]] || {
  print -u2 "应用名称自检失败"
  exit 19
}
...
[[ "$adapter_contract_output" == "AGENT_ADAPTER_CONTRACT_OK products=2 official_icons=ok kimi_work_status=ok workbuddy_public_sessions=ok runtime_first=ok runtime_events=ok invalid_response=ok trae_ide_excluded=ok version_gate=ok isolation=ok presentation=ok" ]] || {
