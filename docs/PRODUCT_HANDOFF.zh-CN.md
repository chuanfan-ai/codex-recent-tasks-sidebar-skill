...
- WorkBuddy 与 TRAE Work 的应用安装/运行状态先同步发布，再后台读取会话等可验证数据；状态栏完成启动后立即重查，目标产品启动或退出时由 macOS 通知即时刷新，30 秒计时器只作兜底。产品标题行直接显示“运行中 / 已安装 / 未安装”，使进程状态与“线程 —”明确分离；后台读取慢或暂不可用时，进程状态仍独立更新，运行态未变化时保留上一份已验证数据，避免周期性闪空。
...
- WorkBuddy/TRAE Work 从未运行到运行的即时状态提升、App 完成启动与目标产品启停事件刷新、后台数据保留，以及运行状态不受会话读取阻塞
